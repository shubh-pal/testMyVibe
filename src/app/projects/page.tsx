...
export default function ProjectsPage() {
  ...
  async function remove(p: ProjectListItem) {
    if (!confirm(`Delete "${p.name}"? This removes all its flows, runs, and issues.`)) return;
    ...
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">Codebases you're auditing with TestMyVibe.</p>
        </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        {projects?.length === 0 && <p className="text-neutral-500 text-sm">No projects yet — create one above.</p>}
        {projects?.map((p) => (
          <div key={p.id} className="card flex items-start justify-between gap-3">
            <Link href={`/projects/${p.id}`} className="min-w-0 flex-1 hover:opacity-80">
              <h2 className="font-medium truncate">{p.name}</h2>
              <p className="text-sm text-neutral-500 mt-0.5 truncate">{p.repoPath || p.repoUrl}</p>
              <p className="text-xs text-neutral-600 mt-2">{p.flows.length} flow(s)</p>
            </Link>
  ...
  async function save() {
    ...
    try {
      const res = await fetch(project ? `/api/projects/${project.id}` : "/api/projects", {
        ...
        body: JSON.stringify({ name, repoPath: repoPath || null, repoUrl: repoUrl || null }),
      });
