...
export function createSchedulingPrompt(
  ...
  return `Set up two recurring quality tasks for this repository using your scheduling feature and the TestMyVibe MCP connection. Discovery is handled by a separate scheduler; do not perform graph discovery in these tasks. If you cannot schedule tasks, explain that limitation and provide these instructions for a supported external scheduler; do not claim scheduling succeeded. Confirm the schedules and timezone after creation.

Capabilities: get_project returns the project and audit playbook; setup_project initializes or resumes graph discovery and reports uncovered graph nodes; save_graph_batch persists a bounded source-evidenced graph update; claim_next_audit atomically claims the next graph journey; report_step_result records source evidence; report_issue creates a pending finding (approved automatically when project auto-approve is enabled); finish_run completes the audit; claim_next_issue atomically claims an approved issue; submit_issue_resolution submits a fix for review (closed automatically when project auto-close is enabled).

TASK 1 — ${auditCadence}: audit pending graph journeys.
